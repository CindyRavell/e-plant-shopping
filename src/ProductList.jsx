...
function ProductList() {
  ...
  const handleRemoveDisable = (product) => {
    setAddedToCart((prevState) => ({
      ...prevState,
      [product.name]: false,
    }));
  ...
  const isPlantAdded = (plantName) => {
    const isPlantAddedTIntoObject =
      Object.keys(addedToCart).includes(plantName);
    return isPlantAddedTIntoObject ? addedToCart[plantName] : false;
  };
    ...
    <div>
      ...
      {!showCart ? (
          ...
          {plantsArray.map((category, index) => (
            <div key={index}>
              <div className="plantname_heading">
                <h1 className="plant_heading" key={category.category}>
                  {category.category}
                </h1>
              </div>
                  ...
                  <div className="product-card" key={plantIndex}>
                    ...
                    <button
                      className={`product-button ${
                        isPlantAdded(plant.name) ? "added-to-cart" : ""
                      }`}
        ...
        <CartItem
          ...
          onDeleteItem={handleRemoveDisable}
        />
      )}
    </div>
  );
